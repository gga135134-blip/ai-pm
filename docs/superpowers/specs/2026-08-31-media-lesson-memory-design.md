...
「末尾」的确切含义：`write_script` 把所有资产拼成**一段一次性发出的长文本**
（约一两千字），顺序是 人设 → 记忆点 → 素材 → 原料 → 角度 → 打法 → 选题
→ 谜题 → 为什么做 → hint → **本子** → `请写出这条内容的口播脚本。`

理由：近因效应——LLM 对靠近提示词末尾的约束更敏感。
红线放开头会被前面一两千字的人设/素材/打法冲淡。
这是经验规律不是铁律，且本轮注入量小（≤5 行）影响有限，
但放末尾无任何坏处。

**为什么不放 `SCRIPT_SYSTEM`（系统提示词）**：系统提示词写死在代码里，
而红线按人设不同、会随时增删。放库里+末尾注入，页面上加一条下次写稿即生效，
不用改代码发版。
...
1. 用户正在跟助手对话——**助手绝不主动发起会话**
2. 用户说的是否定判断（「这样不行」），而非普通改稿要求（「加个案例」）
3. 就在它完成改动的那一次回复里顺带提，**不重复提第二遍**

这与项目③ 自动驾驶的「定时心跳推送」是**完全不同的东西**，本轮不涉及任何
定时/主动推送。本子只在对话里被动沉淀。

**这个度上线后按手感调。** 首版取保守策略（宁可漏记，不要吵）。
用户觉得烦就收紧、觉得漏太多就放宽——都是 `MEDIA_ASSISTANT_SYSTEM` 里
改一句话的事，不值得在设计阶段纠结。
