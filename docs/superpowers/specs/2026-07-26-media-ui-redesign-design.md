...
## 10.A 待办：导入/同步四工具的最终归属（本轮临时安置）

**现状（本轮做法）：** 这四个工具原本堆在内容看板页头，跟「发布」这一步没有关系，本轮先统一收进**体系库面板底部**的「导入 / 同步」分组。其中「视频反向入库」「批量导入老文案」的浮层定义在看板页，所以入口带 `?tool=reverse|legacy` 跳回看板自动打开；`?tool=feishu` 直接触发同步。

**最终归属（用户 2026-07-26 确认）：**

| 工具 | 最终归属 | 理由 |
|---|---|---|
| 从飞书同步 | **复盘** | 拉平台数据回填，是复盘的输入 |
| 未对上的数据 | **复盘** | 同步没匹配上的数据要人工处理，同属数据回填链路 |
| 视频反向入库 | **复盘** | 跟着「已发」走（见下），补录已发内容及其表现 |
| 批量导入老文案 | **留在体系库** | 导的是历史稿子，喂老文案库/打法库，与复盘无关 |

### 「已发」列的归属调整（用户确认）

**当前：** 发布步（`/media/board?view=publish`）显示 `ready`（待发）+ `published`（已发）两列。

**目标：** 发布步**只管「待发」**；`published`（已发）连同「视频反向入库」一起移到**复盘**步。理由：已发内容是复盘的输入，与飞书同步／未对上的数据同属数据回填链路，放一起才是完整闭环；发布步专注「把待发的发出去」这一个动作。

**落地要点（下一轮做）：**
- 发布步筛选条件由 `stage in ['ready','published']` 收窄为 `stage == 'ready'`。
- 复盘页 `/media/review` 增加「已发内容」区块（列 `published` / `reviewed` 的内容，点进 `media_content`），并把三个数据回填工具的入口放在这里。
- `/media/ui/steps` 的 `publish.done` 判定需相应复核：发布步「做完」应以「没有待发积压」而非「有已发」来判定。
- 体系库面板只留「批量导入老文案」，删掉 `?tool=feishu|reverse` 的回跳（浮层就地定义在复盘页）。

**调整时机：** 复盘步骤具备数据回填界面之后（下一轮）。本轮不动，避免半成品状态。
